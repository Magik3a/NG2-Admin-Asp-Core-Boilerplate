...
namespace MultiAPI.Tests
{
    public abstract class MultiAPITestBase : AbpIntegratedTestBase<MultiAPITestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MultiAPITestBase()
        {
        ...
        protected void UsingDbContext(Action<MultiAPIDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MultiAPIDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MultiAPIDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MultiAPIDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MultiAPIDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MultiAPIDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MultiAPIDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MultiAPIDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MultiAPIDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MultiAPIDbContext, Task<T>> func)
        {
            T result;
